...
//#region node_modules/.nitro/vite/services/ssr/assets/seed-kib-DIdffTG_.js
/**
...
async function seedKib(query) {
	const existing = await query("select count(*)::int as c from assets where id like $1", ["kib-%"]);
	...
	for (const s of sites) await query(siteSql, s);
    ...
    on conflict (id) do nothing`;
	const BATCH = 40;
	for (let i = 0; i < assets.length; i += BATCH) for (const a of assets.slice(i, i + BATCH)) await query(assetSql, a);
	console.log(`[seed-kib] inserted ${sites.length} sites, ${assets.length} assets`);
}
/** @param {unknown} result */
function rowCount(result) {
	if (Array.isArray(result)) return result[0]?.c ?? 0;
